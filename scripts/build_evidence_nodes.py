...
NODES = [
    ...
    ("B0.hc_nn_no_constraint", "B0", "B0", "B0.3 HC-NN 无约束", "HC-NN no-constraint",
     "evidence", False, N(HC, eq("use_ci_penalty", False), eq("use_w_constraints", False)), ["B0.hc_nn.er"]),
    ("B0.hc_w_only", "B0", "B0", "B0.4 HC W-only", "HC W-only", "evidence", False,
     N(HC, eq("use_w_constraints", True), eq("use_ci_penalty", False)), ["B0.hc_w_only.er"]),
    ...
    ("A1.ALM_ALL", "A1", "A1", "A1.1 ALM-all（全部 CE 走 ALM）", "ALM-all", "code", False,
     eq("ce_constraint_backend", "alm_all"), ["A1.ALM_ALL.er"]),
    ("A1.PBM_ALL", "A1", "A1", "A1.2 PBM-all（全部 CE 走 PBM）", "PBM-all", "code", False,
     eq("ce_constraint_backend", "pbm_all"), ["A1.PBM_ALL.er"]),
    ...
    ("A1.STOCHASTIC_PBM", "A1", "A1", "A1.4 stochastic-PBM", "Stochastic PBM", "code", False,
     eq("ce_pbm_backend", "stochastic_pbm"), ["A1.STOCHASTIC_PBM.er"]),
    ("A1.SCO_LAYER", "A1", "A1", "A1.5 SCO 层（随机约束优化 + proximal/adaptive）",
     "SCO layer", "code", False, eq("stochastic_constrained", True), ["A1.SCO_LAYER.er"]),
    ...
    ("A3.covariance", "A3", "A3", "A3.1 残差协方差（条件期望）", "residual covariance (cond. expectation)",
     "evidence", True, eq("ci_penalty_kind", "conditional_expectation"), ["A3.covariance.er"]),
    ("A3.partial_correlation", "A3", "A3", "A3.2 标准化偏相关（按数据族拆分）", "standardized partial correlation (by family)",
     "evidence", True, eq("ce_statistic_kind", "partial_correlation"), []),
    ("A3.pcorr.linear_ER", "A3.partial_correlation", "A3", "A3.2a 线性 ER", "linear ER",
     "evidence", True, eq("ce_statistic_kind", "partial_correlation"),
     ["C0b.ce_vs_nn.ER", "C0b.ce_vs_nn.repair_ER", "C0b.ce_vs_nn.priority_ER"]),
    ("A3.pcorr.nonlinear_ER", "A3.partial_correlation", "A3", "A3.2b 非线性 ER", "nonlinear ER",
     "evidence", True, eq("ce_statistic_kind", "partial_correlation"),
     ["P1.3.nonlin_ce_vs_nn.ER", "P1.3.nonlin_ce_vs_nn.repair_ER"]),
    ("A3.pcorr.SF", "A3.partial_correlation", "A3", "A3.2c SF", "SF",
     "evidence", True, eq("ce_statistic_kind", "partial_correlation"), ["C0b.ce_vs_nn.SF"]),
    ...
    ("A4.graph_quality", "A4", "A4", "A4.7 图质量（F1/SHD/edge count）", "graph quality (F1/SHD/edges)",
     "code", True, lambda d: pd.to_numeric(d.edge_f1, errors="coerce").notna(), []),
    ("A4.constraint_quality", "A4", "A4", "A4.8 约束质量（indep/dep count）", "constraint quality",
     "code", True, lambda d: pd.to_numeric(d.indep_constraints, errors="coerce").notna(), []),
    ("A4.solver_stability", "A4", "A4", "A4.9 求解稳定性（status/gap/time）", "solver stability",
     "code", True, lambda d: pd.to_numeric(d.n_folds_graph, errors="coerce").fillna(0) > 0, []),
    ...
    ("A5.hidden_depth", "A5", "A5", "A5.2 hidden / depth", "hidden / depth", "code", True,
     lambda d: d.hidden_dim.notna() & d.hidden_dim.ne(""), ["A5.hidden_depth.er"]),
    ("A5.lr_wd", "A5", "A5", "A5.3 learning rate / weight decay", "lr / weight decay", "code", True,
     lambda d: d.learning_rate.notna() & d.learning_rate.ne(""), ["A5.lr_wd.er"]),
    ("A5.n_outer_inner", "A5", "A5", "A5.4 n_outer / n_inner", "n_outer / n_inner", "code", True,
     lambda d: d.n_outer.notna() & d.n_outer.ne(""), ["A5.n_outer_inner.er", "A5.n_outer_inner_matched.er"]),
    ...
    ("A6.lag", "A6", "A6", "A6.2 feature lag", "feature lag", "code", True,
     lambda d: pd.Series(False, index=d.index), ["A6.lag.fred"]),
    ("A6.trend", "A6", "A6", "A6.3 time trend", "time trend", "code", True,
     lambda d: pd.Series(False, index=d.index), ["A6.trend.fred"]),
    ("A6.regime", "A6", "A6", "A6.4 regime break", "regime break", "code", True,
     lambda d: pd.Series(False, index=d.index), ["A6.regime.fred"]),
    ("A6.huber", "A6", "A6", "A6.5 Huber 鲁棒损失", "Huber robust loss", "code", True,
     eq("prediction_loss", "huber"), ["A6.huber.fred"]),
