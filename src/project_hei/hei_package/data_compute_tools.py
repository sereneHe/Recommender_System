...
Original file is located at
    https://colab.research.google.com/drive/1uyS4OUgv5WEPp6QZVoz94Jpi1Keeh4PF
"""
...
if __name__ == "__main__":
    np.random.seed(42)
    prep_data = pd.DataFrame({'ID': range(1, 21), 'TRIG (mg/dL)': np.random.normal(150, 30, 20)})
    objective_df = pd.DataFrame({'ID': range(1, 21), 'feat1': np.random.normal(0, 1, 20), 'feat2': np.random.normal(0, 1, 20)})
    get_pca(prep_data, objective_df, 'test_pca', n_dims=2, label_col='TRIG (mg/dL)', do_print=False)
