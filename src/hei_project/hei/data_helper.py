...
def read_HEI():
    if os.path.exists(os.path.join(BASE_PATH, "data", "HEI", "hei.pkl")):
        return pd.read_pickle(os.path.join(BASE_PATH, "data", "HEI", "hei.pkl"))

    fped = pd.read_csv(os.path.join(BASE_PATH, "data", "HEI", "processed_fped.csv"))
    diet = pd.read_csv(os.path.join(BASE_PATH, "data", "HEI", "processed_diet.csv"))
    ...
    hei_data.to_pickle(os.path.join(BASE_PATH, "data", "HEI", "hei.pkl"))
    return hei_data
...
def read_body_comp():
    if os.path.exists(os.path.join(BASE_PATH, "data", "body_composition", "body_comp.pkl")):
        return pd.read_pickle(os.path.join(BASE_PATH, "data", "body_composition", "body_comp.pkl"))

    body_comp = pd.read_excel(os.path.join(BASE_PATH, "data", "body_composition", "BiosensorsMicrocaya_data_combined_jan2025.xlsx"))
    ...
    body_comp.to_pickle(os.path.join(BASE_PATH, "data","body_composition", "body_comp.pkl"))
    body_comp['gender_numeric'] = body_comp['gender'].apply(lambda x: 0 if x == 'Male' else 1)
    return body_comp

def read_blood_data():
    if os.path.exists(os.path.join(BASE_PATH, "data","UpdatedDataFromSara", "blood_data.pkl")):
        return pd.read_pickle(os.path.join(BASE_PATH, "data", "UpdatedDataFromSara","blood_data.pkl"))

    file = os.path.join(BASE_PATH, "data", "UpdatedDataFromSara", "biochemical data all converted values.xlsx")
    ...
    with open(os.path.join(BASE_PATH, "data", "UpdatedDataFromSara", "blood_data.pkl"), "wb") as f:
        pickle.dump(blood_data, f)
    with open(os.path.join(BASE_PATH, "data", "UpdatedDataFromSara", "site_data.pkl"), "wb") as f:
        pickle.dump(site_data, f)

    return blood_data

def read_average_expenditure():
    # os.path.exists(os.path.join(BASE_PATH, "data",  "energy_expenditure", "expenditure.pkl")) and
    if os.path.exists(os.path.join(BASE_PATH, "energy_expenditure", "average_expenditure.pkl")):
        return pd.read_pickle(os.path.join(BASE_PATH, "energy_expenditure", "average_expenditure.pkl"))

    energy_expenditure = pd.DataFrame()
    for file in os.listdir(os.path.join(BASE_PATH, "data","energy_expenditure")):
        if file.endswith(".csv"):
            tee = pd.read_csv(os.path.join(os.path.join(BASE_PATH, "data", "energy_expenditure"), file))
    ...
    average_expenditure.to_pickle(os.path.join(BASE_PATH, "data", "energy_expenditure", "average_expenditure.pkl"))
    energy_expenditure.to_pickle(os.path.join(BASE_PATH, "data" ,"energy_expenditure", "expenditure.pkl"))
...
def load_all_data():
    ...
    gmwi_data = pd.read_csv(os.path.join(BASE_PATH, "data", "gmwi_data.csv"))
    ...
    ##### Blood Pressure
    data = pd.read_excel(os.path.join(BASE_PATH, "data", "UpdatedDataFromSara", "Blood pressure values all sites WP2.xlsx"))
    ...
    ##### microbiome alpha
    microbiome_alpha_df = pd.read_csv(os.path.join(BASE_PATH, "data", "microbiome","alpha_summary_CoDiet_total_v2.csv"))
    ...
    scafs_df = cleaned_loader(os.path.join(BASE_PATH, "data", "more_biomarkers","scafs-stool.csv"), 'scafs')
    scafs_df = scafs_df[['ID', 'VISIT', 'scafs_acetate', 'scafs_butyrate', 'scafs_formate', 'scafs_propionate']]

    ms_urine_df = cleaned_loader(os.path.join(BASE_PATH, "data", "more_biomarkers","ms-urine.csv"), 'ms_urine')
    ms_urine_df = ms_urine_df.drop(columns=['ms_urine_type', 'ms_urine_sample-type'])

    ms_serum_df = cleaned_loader(os.path.join(BASE_PATH, "data","more_biomarkers","ms-serum.csv"), 'ms_serum')
    ms_serum_df = ms_serum_df.drop(columns=['ms_serum_type', 'ms_serum_sample-type'])

    nmr_urine_df = cleaned_loader(
        os.path.join(BASE_PATH, "data","UpdatedNMRLipids_12_25","unified-nmr-targeted-urine_v2.xlsx"), 'nmr_urine',
    ...
    df = pd.read_excel(os.path.join(BASE_PATH, "data","lipidomics","lipidomics.xlsx"))
    df = df[df['type'] == 'sample']
    df.drop(columns=['type'], inplace=True)
    lipidomics_df = cleaned_loader('', 'ms_lip', reader_func=lambda fn: df)

    lipidomics_dbs_rbc_df = cleaned_loader(os.path.join(BASE_PATH, "data","lipidomics","lipidomics-dbs-rbc.xlsx"),
                                           'dbs_rbc_lip',
                                           reader_func=pd.read_excel)

    microbiome_cl_df = pd.read_csv(os.path.join(BASE_PATH, "data","derived","microbiome_4_clusters.csv"))
    microbiome_cl_df = microbiome_cl_df.drop(columns=['Unnamed: 0'])

    microbiome_phyl_cl_df = pd.read_csv(os.path.join(BASE_PATH, "data","derived","microbiome_phylumn4_clusters.csv"))
    microbiome_phyl_cl_df = microbiome_phyl_cl_df.drop(columns=['Unnamed: 0'])

    microbiome_embedding_df = pd.read_csv(os.path.join(BASE_PATH, "data","derived","microbiome_embedding_20.csv"))
    microbiome_clean15_df = pd.read_csv(os.path.join(BASE_PATH, "data","derived","microbiome_clean15.csv"))
