...
from hei_project.hei.mk_utils import clean, corr, perm_test_pval, average_visits, split_visits_from_column


from hei_project.hei.mk_utils import MedianKNNRegressor, percentile_mask, corr, perm_test_pval
...
def compute_predictor_errors(prep_data, hei_feats, target_col, w_est, row_and_col_names,
    ...
    if custom_objective == 'lagrange':
        present_feats = [f for f in hei_feats if f in row_and_col_names_indices]
        missing_feats = [f for f in hei_feats if f not in row_and_col_names_indices]
        if missing_feats:
            print(
                f"[WARN] {len(missing_feats)} feature(s) missing from row_and_col_names; "
                f"skipping in W_est alignment. Example: {missing_feats[:5]}"
            )
        if len(present_feats) == 0:
            raise KeyError("None of the selected features were found in row_and_col_names.")
        idx_list = [row_and_col_names_indices[f] for f in present_feats]
    ...
    if model_name == 'XGB':
        if XGBRegressor is None:
            raise ImportError(
                ...
                "Use model_name='REG' or fix libomp/xgboost installation."
            )
