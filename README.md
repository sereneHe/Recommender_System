# Machine Learning Operations for CoDiet Nutrition Modeling
...
1. Build CoDiet nutrition prediction workflows from merged dietary, biomarker, body-composition, and energy-expenditure tables.
2. Create automated and reproducible pipelines for preparation, training, evaluation, and visualization of nutrition outcomes.

### Data
#### Dataset
The project is based on CoDiet raw data assets in `datasets/raw`:
- `HEI/hei.pkl`
- `UpdatedDataFromSara/blood_data.pkl`
- `body_composition/body_comp.pkl`
- `energy_expenditure/average_expenditure.pkl`

#### Modality
The data is tabular and merged on participant `ID`. The merged `prep_data` table is used for feature selection and target-wise regression.

### Models

The active pipeline is a CoDiet recommender workflow using regression models (`REG`/`XGB`) with forward feature selection and variance-reduction reporting.
...
## Notebook Env Help
Notebook hard-coded variables are exposed via CLI options + environment variables.

Show top-level help:
```bash
uv run python -m hei_project.notebook_env --help
```

Show per-command help:
```bash
uv run python -m hei_project.notebook_env prepare --help
uv run python -m hei_project.notebook_env train --help
uv run python -m hei_project.notebook_env patterns --help
```

Main mappings:
- `DO_PREP` -> `HEI_DO_PREP`
- `N_SELECT_FEATURES` -> `HEI_N_SELECT_FEATURES`
- `n_runs` -> `HEI_N_RUNS`
- `target_columns` -> `HEI_TARGETS`
- `model_name` -> `HEI_MODEL_NAME`
- `directory` and regex pattern(s) -> `HEI_PKL_OUTS_DIR`, `HEI_PATTERNS`
